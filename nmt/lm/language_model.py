...
class LM(object):
    def __init__(self, hps, mode="train", ps_device="/gpu:0", standardalone=False,
                 x_i=None, y_i=None, w_i=None):
        self.hps = hps
        ...
        batch_losses = []
        tower_grads = []
        with tf.control_dependencies([self.x, self.y, self.w]):
        ...
        for i in range(hps.num_gpus):
            with tf.device(assign_to_gpu(i, ps_device)), tf.variable_scope(tf.get_variable_scope(),
                ...
                loss, batch_loss = self._forward(i, xs[i], ys[i], ws[i])
                losses += [loss]
        ...
        self.batch_loss = tf.add_n(batch_losses) / len(losses)
    ...
    def _forward(self, gpu, x, y, w):
        ...
        if hps.num_sampled == 0:
            ...
            loss = tf.nn.sampled_softmax_loss(softmax_w, softmax_b,
                                              targets, tf.to_float(inputs), hps.num_sampled, hps.vocab_size)
        #loss = tf.Print(loss,[loss],summarize=hps.batch_size*hps.num_steps)
        batch_loss = tf.reduce_mean(
            tf.reshape(loss * tf.reshape(w, [-1]),[hps.batch_size,-1]),[1])
