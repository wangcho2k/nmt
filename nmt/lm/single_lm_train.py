...
def main(_):
    ...
    vocab = Vocabulary.from_file("vocab.30k.en")

    if FLAGS.mode == "train":
        hps.batch_size = 256
        dataset = Dataset(vocab, FLAGS.datadir + "/training-monolingual.tokenized.shuffled.pruned/*")
        ...
        if FLAGS.mode.startswith("eval_train"):
            data_dir = FLAGS.datadir + "/training-monolingual.tokenized.shuffled.pruned/*"
        else:
